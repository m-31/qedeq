...
<!-- Checkstyle XML Style Sheet by Stephane Bailliez <sbailliez@xxxxxxxxxx>         -->
...
<!--            style="${doc.dir}/checkstyle-noframes-detailed.xsl"/>               -->
...
<xsl:template match="checkstyle">
      ...
      <tr>
		...
		<xsl:variable name="infoCount" select="count(error[@severity='info'])"/>
          ...
          <xsl:call-template name="alternated-row"/>
					<td><a href="#f-{translate(@name, '\', '_')}"><xsl:value-of select="@name"/></a></td>
    ...
    <table class="log" border="0" cellpadding="5" cellspacing="2" width="100%">
        ...
        <xsl:call-template name="alternated-row"/>
    	  <td>        
    	  	<xsl:call-template name="severity-level"/>
  ...
  <xsl:template name="severity-level">
    <xsl:attribute name="class">
