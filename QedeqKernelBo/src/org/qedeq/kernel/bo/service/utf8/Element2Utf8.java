 ...
 */
public final class Element2Utf8 extends AbstractModuleVisitor {
     ...
     */
    public Element2Utf8(final ModuleReferenceList references) {
        ...
        public String getLatex(final ElementList list, final boolean first) {
            ...
            if (list.size() > 1) {
                ...
                for (int i = 1; i < list.size(); i++) {
                    buffer.append(Element2Utf8.this.getLatex(list.getElement(i), false));
                    if (i + 1 < list.size()) {
                        buffer.append(", ");
                    }
                }
    ...
    class Predcon implements ListType {
        public String getLatex(final ElementList list, final boolean first) {
            ...
            PredicateDefinition definition = (PredicateDefinition)
                Element2Utf8.this.getPredicateDefinitions().get(identifier);
            if (definition == null) {
                // try external modules
                try {
                    final int external = name.indexOf(".");
                    if (external >= 0 && Element2Utf8.this.getReferences() != null
                            && Element2Utf8.this.getReferences().size() > 0) {
                        final String label = name.substring(0, external);
                        final DefaultKernelQedeqBo newProp = (DefaultKernelQedeqBo)
                            Element2Utf8.this.getReferences().getQedeqBo(label);
                        if (newProp != null) {
            ...
            if (definition != null) {
                ...
                for (int i = list.size() - 1; i >= 1; i--) {
                    StringUtility.replace(define, "#" + i, Element2Utf8.this.getLatex(
    ...
    class Funcon implements ListType {
        ...
        public String getLatex(final ElementList list, final boolean first) {
            ...
            FunctionDefinition definition = (FunctionDefinition)
                Element2Utf8.this.getFunctionDefinitions().get(identifier);
            if (definition == null) {
                // try external modules
                try {
                    final int external = name.indexOf(".");
            ...
            if (definition != null) {
                final StringBuffer define = new StringBuffer(definition.getLatexPattern());
                for (int i = list.size() - 1; i >= 1; i--) {
                    StringUtility.replace(define, "#" + i, Element2Utf8.this.
    ...
    class QuantorIntersection implements ListType {
        public String getLatex(final ElementList list, final boolean first) {
            ...
            buffer.append(prefix);
            if (0 < list.size()) {
                buffer.append("{").append(Element2Utf8.this.getLatex(list.getElement(0), false))
                .append("}");
            }
            for (int i = 1; i < list.size(); i++) {
                buffer.append(Element2Utf8.this.getLatex(list.getElement(i), false));
